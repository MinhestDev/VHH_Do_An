	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'Bạn đã hẹn giờ cho ngày hôm nay';
				}
				else{
					//updates
					...
					if($conn->query($sql)){
						$output['message'] = 'Thời gian trong: '.$row['firstname'].' '.$row['lastname'];
					}
					else{
						$output['error'] = true;
				...
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'Không thể hết thời gian chờ. Không có thời gian vào.';
				}
				else{
					$row = $query->fetch_assoc();
					if($row['time_out'] != '00:00:00'){
						...
						if($conn->query($sql)){
							$output['message'] = 'Hết giờ: '.$row['firstname'].' '.$row['lastname'];
			...
			$output['error'] = true;
			$output['message'] = 'ID nhân viên không tìm thấy.';
		}
